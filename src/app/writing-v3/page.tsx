...
export default function WritingV3Page() {
  return (
    <div className="page-enter" style={{ padding: '60px 0' }}>
      <div className="container">
        ...
        {/* Accordion sections with counts */}
        <div style={{ borderTop: '1px solid var(--color-border)' }}>
          <Accordion title={`Books (${books.length})`} defaultOpen={true}>
          ...
          <Accordion title={`Essays (${essays.length})`}>
          ...
          <Accordion title={`Opinion (${opinion.length})`}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {opinion.map((item, i) => (
                item.url && item.url !== '#'
                  ? <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={linkItemStyle}>{item.text}</a>
          ...
          <Accordion title={`Journal Articles (${journalArticlesPeerReviewed.length + journalArticlesNonPeerReviewed.length})`}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Accordion title={`Reports (${reports.length})`}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {reports.map((item, i) => (
                item.url && item.url !== '#'
                  ? <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={linkItemStyle}>{item.text}</a>
          ...
          <Accordion title={`Book Chapters (${bookChapters.length})`}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
