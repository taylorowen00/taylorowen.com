...
const recentArticles = [
  ...
  { title: 'It\'s time to table the Online Harms Bill.', pub: 'The Toronto Star', date: 'November 3, 2023', coauthor: 'Emily Laidlaw', url: 'https://www.thestar.com/opinion/contributors/its-time-to-table-the-online-harms-bill/article_0eeb5ac6-8e39-11ee-a256-3379f1b68c68.html' },
  { title: 'We are in a crisis of knowledge - and it couldn\'t have happened at a worse time.', pub: 'The Globe and Mail', date: 'October 21, 2023', url: 'https://www.theglobeandmail.com/opinion/article-we-are-in-a-crisis-of-knowledge-and-it-couldnt-have-happened-at-a/' },
  { title: 'How we can move forward, now that the divisive Bill C-18 is law.', pub: 'The Globe and Mail', date: 'July 21, 2023', coauthor: 'Supriya Dwivedi', url: 'https://www.theglobeandmail.com/opinion/article-how-we-can-move-forward-now-that-the-divisive-bill-c-18-is-law/' },
  { title: 'Don\'t Ban TikTok, Make it Safer.', pub: 'The Globe and Mail', date: 'May 26, 2023', coauthor: 'Frances Haugen', url: 'https://www.taylorowen.com/s/Dont-ban-TikTok-Make-it-safer.pdf' },
  { title: 'The Online News Act keeps journalism alive while it adapts to a new world.', pub: 'National Post & The Hub', date: 'November 8, 2022', url: 'https://nationalpost.com/opinion/taylor-owen-the-online-news-act-keeps-journalism-alive-while-it-adapts-to-a-new-world' },
  { title: 'Canada\'s Online News Act shows how other countries are learning from Australia\'s news bill.', pub: 'NiemanLab', date: 'August 9, 2022', coauthor: 'Supriya Dwivedi', url: 'https://www.niemanlab.org/2022/08/canadas-online-news-act-shows-how-other-countries-are-learning-from-australias-news-bill/' },
  { title: 'Mis- and Disinformation During the 2021 Canadian Federal Election.', pub: 'Media Ecosystem Observatory', date: 'March 31, 2022', url: 'https://www.mediatechdemocracy.com/all-work/mis-and-disinformation-during-the-2021-canadian-federal-election' },
  { title: 'We have the regulatory tools we need to fix Facebook.', pub: 'The Globe and Mail', date: 'October 13, 2021', coauthor: 'Beverley McLachlin and Peter MacLeod', url: 'https://www.theglobeandmail.com/opinion/article-we-have-the-regulatory-tools-we-need-to-fix-facebook/' },
  { title: 'Canada shouldn\'t turn away from the difficult task of regulating online speech.', pub: 'The Globe and Mail', date: 'June 25, 2021', url: 'https://www.theglobeandmail.com/opinion/article-canada-shouldnt-turn-away-from-the-difficult-task-of-regulating-online/' },
  { title: 'The Case for Platform Governance.', pub: 'The Centre for International Governance Innovation', date: 'November 4, 2019', url: 'https://www.taylorowen.com/s/The-Case-for-Platform-Governance.pdf' },
]
...
function AccordionSection({
  ...
  return (
    <div style={{ marginBottom: '8px' }}>
      {/* Header bar with tinted background */}
      <button
      ...
      >
        ...
        <span
          style={{
            ...
            borderLeft: '8px solid var(--color-text)',
      ...
      <div
        style={{
          overflow: 'hidden',
          maxHeight: open ? '200000px' : '0',
          transition: open ? 'max-height 0.5s ease-in' : 'max-height 0.35s ease-out',
        }}
      >
...
export default function WritingV5Page() {
  return (
    <div className="page-enter" style={{ padding: '60px 0' }}>
      <div className="container">
          ...
          {recentArticles.map((article, i) => (
            <a
              ...
              style={{
                display: 'block',
                padding: '12px 0',
                borderBottom: i < recentArticles.length - 1 ? '1px solid var(--color-border-light)' : 'none',
                color: 'var(--color-text)',
                textDecoration: 'none',
                fontSize: '15px',
                lineHeight: 1.6,
            ...
            >
              &ldquo;<span style={{ color: 'var(--color-accent)' }}>{article.title}</span>&rdquo;
        ...
        {/* ── Accordion sections (Chevron + Tinted Background) ──── */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0px' }}>
          {/* Books */}
          <AccordionSection title="Books" count={books.length}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {books.map((book, i) => (
                <div key={i}>
                  <a href={book.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '16px', fontStyle: 'italic', color: 'var(--color-accent)' }}>{book.title}</a>
          ...
          {/* Essays */}
          <AccordionSection title="Essays" count={essays.length}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {essays.map((item, i) => (
                <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={linkItemStyle}>{item.text}</a>
              ))}
            </div>
          </AccordionSection>

          {/* Opinion */}
          <AccordionSection title="Opinion" count={opinion.length}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {opinion.map((item, i) => (
                item.url && item.url !== '#'
                  ? <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={linkItemStyle}>{item.text}</a>
                  : <p key={i} style={itemStyle}>{item.text}</p>
              ))}
            </div>
          </AccordionSection>

          {/* Journal Articles */}
          <AccordionSection title="Journal Articles" count={journalArticlesPeerReviewed.length + journalArticlesNonPeerReviewed.length}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Reports */}
          <AccordionSection title="Reports" count={reports.length}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {reports.map((item, i) => (
                item.url && item.url !== '#'
                  ? <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={linkItemStyle}>{item.text}</a>
          ...
          {/* Book Chapters */}
          <AccordionSection title="Book Chapters" count={bookChapters.length}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
